...
$query = "SELECT * FROM pengumuman ORDER BY idinfo";
$sql = mysql_query ($query);
$i = 0;
// $data = array();
while ($row = mysql_fetch_assoc($sql)) {
	$i++;
	$query2 = "SELECT * FROM admin where id = ".$row["update_by"]." ORDER BY id";
	...
	$output .= '<tr>  
                  <td width="3%">'.$i.'</td>  
                  <td width="3%">'.$row["idinfo"].'</td>  
                  <td width="15%">Nama : '.$namam.'<br>Id Admin : '.$row["update_by"].'<br>Email : '.$emailm.'</td>  
                  <td width="12%">'.$row["tittle"].'</td>  
                  <td width="42%">'.$row["detail_info"].'</td> 
                  <td width="15%"><img src="http://si-mice.com/simice/files/pic/pengumuman/'.$row["infopic"].'" border="0" height="50" width="50" /></td>  
...
require_once('tcpdf/tcpdf.php');  
      ...
      $obj_pdf->SetTitle("Laporan Data Pengumuman Aplikasi SI - MICE");  
      ...
      $content = '';  
      $content .= '  
      <h1 align="center">Laporan Data Pengumuman Aplikasi SI - MICE</h1><br /><br />  
      ...
      $obj_pdf->Output('datapengumumanSIMICE.pdf', 'I');
?>